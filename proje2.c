...
int main(void){
  ...
  *BLKSIZ = 8;  // 8 bytes per block
  ...
  volatile unsigned int *TARGET_MEMORY_ADDRESS = 0x20000000;
  ...
  while(flag){
    ...
    while (numAvailable != 0){
      volatile unsigned int readData = *DATA;
      *TARGET_MEMORY_ADDRESS = readData;
	  hexDisp(*TARGET_MEMORY_ADDRESS);
